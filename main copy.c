...
void solveCrossword(char **words,int words_size, int index, mapStruct crossWord ){//todo non square map of 'x' and 'o'
//    printArray(crossWord.map,crossWord.x_size);
//    printf("\n");
   if(!checkEmpty(crossWord)){// jei masyvas uzpildytas arba zodziai pasibaige tada saugoti
    ...
    if(index < words_size){
        ...
        for (int i = 0; i < crossWord.x_size; i++) {//y_size perhaps TODO
